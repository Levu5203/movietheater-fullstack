  ...
  implements OnInit
{
  ...
  constructor(
    ...
    private readonly movieAdminService: IMovieAdminServiceInterface
  ) {
    super();
  }
  ...
  public delete(id: string): void {
    this.movieAdminService.delete(id).subscribe({
      next: () => {
          this.searchData();
      },
      error: (error) => {
        // Neu xoa khong duoc thi hien thi thong bao loi
        this.showErrorMessage = true;
        this.errorMessage =
          error.error?.message || 'An error occurred while creating the movie';
      },
    });
  }
